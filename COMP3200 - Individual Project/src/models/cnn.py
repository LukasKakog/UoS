...
model.add(Dropout(0.125))  # Dropout layer
...
data_train, data_val, cd_indices_train, cd_indices_val = train_test_split(data, cd_indices, test_size=0.25, random_state=1)

print ("Starting model training...")

# Train the model on the training data and validate on the validation data
start_time = time.time()
model.fit(data_train, cd_indices_train, validation_data=(data_val, cd_indices_val), epochs=100, batch_size=32, callbacks=[time_callback])
end_time = time.time()
