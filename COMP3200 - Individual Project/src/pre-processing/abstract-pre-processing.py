...
with open('SSNDataset\SSN\papers.SSN.jsonl', 'r') as f, open('training_set_abstracts.jsonl', 'w') as outfile:
    for line in f:
        ...
        if isinstance(text, list):
            if all(isinstance(i, list) for i in text):
                text = ' '.join(sum(text, []))
            else:
                text = ' '.join(text)
